...
class CGAN():
    def __init__(self, flags):
        ...
        if self.flags.name is None:
            model_path = '../saved_model/cgan'
        else: 
            model_path = '../saved_model/' + self.flags.name

        self.images_path = '../images/%s' % ('cgan' if self.flags.name is None else self.flags.name)
    ...
    def build_discriminator(self):
        ...
        inp_lab = Input([self.num_classes])
        label_embedding = Flatten()(Embedding(self.num_classes, np.prod(self.img_shape))(inp_lab))
        flat_img = Flatten()(inp)

        model_inp = multiply([flat_img, label_embedding])
